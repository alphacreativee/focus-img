...
// Lấy data từ HTML có sẵn thay vì từ JS array
const imageElements = document.querySelectorAll(".spotlight-img img");
const titleElements = document.querySelectorAll(".spotlight-titles h1");

// Tạo array chứa thông tin từ HTML elements
const spotlightItems = [];
titleElements.forEach((titleEl, index) => {
  const imgEl = imageElements[index];
  if (imgEl) {
    spotlightItems.push({
...
// Set initial opacity cho images
...
ScrollTrigger.create({
  ...
  onUpdate: (self) => {
    ...
    if (progress <= 0.2) {
      ...
      const totalDisstance = startPosition - targetPosition;
      const currentY = startPosition - switchProgress * totalDisstance;
      ...
      if (closestIndex !== currentActiveIndex) {
        ...
        // Sử dụng data từ HTML thay vì hardcoded array
        const bgImgElement = document.querySelector(".spotlight-bg-img img");
        if (bgImgElement && spotlightItems[closestIndex]) {
